...
const SaleChances = ({ data }: SaleChancesProps) => (
        ...
        {data.map((row, idx) => (
          <TableRow key={idx}>
            <TableCell className="border p-2 text-blue-600 hover:underline">
              <Link href="#" tabIndex={0} aria-label={`Details zu ${row.titel}`}>{row.titel}</Link>
            </TableCell>
            <TableCell className="border p-2 text-blue-600 hover:underline">
              <Link href="#" tabIndex={0} aria-label={`Kunde ${row.kunde}`}>{row.kunde}</Link>
            </TableCell>
            ...
            <TableCell className="border p-2 text-center">{row.angebote}</TableCell>
