...
const KalkulationForm = () => {
  ...
  const handleChange = (key: keyof KalkulationState) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    setValues(v => ({ ...v, [key]: key === "bemerkung" ? e.target.value : Number(e.target.value) }))
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        ...
        <div className="space-y-4">
          {labels.map(({ key, label }) => (
            <label key={key} className="flex flex-col gap-1 text-sm font-medium" htmlFor={key}>
              {label}
              <Input
                id={key}
                type="number"
        ...
        <div className="space-y-4 pt-7">
          {labels.map(({ key }) => (
            <div key={key} className="text-right text-sm text-gray-700 pt-2">
